...
function mergeRulesets(base: Ruleset, flavor: Ruleset): Ruleset {
    ...
    const mergeContent = (baseContent: (Section | Article)[], flavorContent: (Section | Article)[]) => {
...
export async function getMergedRuleset(slug: string): Promise<Ruleset | null> {
    ...
    if (!baseData) {
        throw new Error(`Base ruleset "${flavorData.baseRulesetSlug}" not found for flavor "${slug}".`);
    }
